...
function main() {
  ...
  ///files for each whitelisted address
  // import distribution from this file
  const filename = "gen_files/Addresses.csv";

  // what file should we write the merkel proofs too?
  const output_file = "gen_files/MerkleProof.json";

  //file that has the user white list
  const userclaimFile = "gen_files/Whitelist.json";
  ...
  fs.createReadStream(filename)
    .pipe(csv())
    .on("data", (row) => {
      const user_dist = [row["user_address"]]; // create record to track user_id of leaves
